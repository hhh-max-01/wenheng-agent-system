...
**在线演示：** https://wenheng-agent-system.onrender.com

> 使用 Render 免费实例，长时间无人访问后首次打开可能需要约 50 秒唤醒，请稍候并刷新页面。

## 1. 系统能做什么

- 选择两类数据集：计划任务书、立项申请书。
- 上传一个或多个 DOC、DOCX、PDF、JSON、CSV、TXT、Markdown 文件。
- 从 JSON 数组或 CSV 表格中拆分多条数据。
- 分析用户 Intent，只选择相关判断规则。
- 使用 Spec-handoff 两阶段流程：先提取人员、日期、预算、指标和冲突，再把事实交接单交给裁决智能体。
- 对长文档做章节感知压缩，优先保留表格、重复字段和可能导致不通过的证据，同时保留文档首尾。
- 从 `review_memory.json` 检索通用审核经验；记忆规则与证据模式，不记训练文件名和标签。
- 先执行确定性预检查，再让大模型完成语义分析和裁决。
- 输出固定 JSON 结构。
- API 不可用时自动降级为本地规则演示模式。
- 在页面展示每条记录的规则、证据、理由和置信度。

> 旧版 `.doc` 优先由 Office Oxide 直接读取原始二进制文件；读取失败或正文不足时，系统才在临时目录中调用 LibreOffice 转换。原始上传文件不会被覆盖，临时文件会自动删除。

批量上传时，每个文件独立解析。单个文件读取失败会显示独立的“处理失败”结果，不会中断同批其他文件。

DOCX 会先进行结构完整性检查。普通 CRC 校验异常会自动重建；结构损坏时优先调用 LibreOffice 修复，仍无法完整修复时仅恢复可读正文，并按“材料完整性无法确认”进入审核规则，而不是向用户显示 `BadZipFile`。
...
3. Render会读取`render.yaml`并创建Docker服务；Office Oxide负责直接读取旧版DOC，容器内的LibreOffice作为备用转换器。
...
- 严重损坏或加密的 Word 文件可能只能恢复部分文字；系统会给出中文完整性提示并保守裁决。
- Render 免费实例首次构建 Docker 镜像和闲置唤醒所需时间会比纯 Python 服务稍长。
- PDF 为扫描图片且没有文本层时，需要额外 OCR，目前不会自动识别图片文字。
- 原始提取文本最多保留120,000字符；发送给模型前压缩到约24,000字符，极端长文档仍可能丢失低相关内容。
